...
def hsv2rgb3d(vol):
    ...
    Hrange2 = np.bitwise_and(
        vol[0, :, :, :] >= np.pi / 3, vol[0, :, :, :] < np.pi * 2 / 3
    )
    Hrange3 = np.bitwise_and(vol[0, :, :, :] >= np.pi * 2 / 3, vol[0, :, :, :] < np.pi)
    Hrange4 = np.bitwise_and(vol[0, :, :, :] >= np.pi, vol[0, :, :, :] < np.pi * 4 / 3)
    Hrange5 = np.bitwise_and(
        vol[0, :, :, :] >= np.pi * 4 / 3, vol[0, :, :, :] < np.pi * 5 / 3
    )
...
def convertToFan(vec, halfSphere=False, weights=None, mask=None):
    ...
    colormap_vol = np.vstack(
        (colormap_vol, np.ones_like(colormap_vol[0, :, :, :])[None, :])
    )
...
def holeFillGauss(vol, thresh=5, minBlobSize=50, dilSize=5):
    ...
    blobSize = scipy.ndimage.sum(
        holeMask, labels, np.arange(0, np.max(labels)).tolist()
    )
    holeMask = np.isin(labels, np.where(blobSize > minBlobSize))
    # Small dilation to be safe
    holeMask = scipy.ndimage.binary_dilation(
        holeMask, np.ones((dilSize, dilSize, dilSize))
    )
